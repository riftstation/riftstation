...
if [ "$BUILD_TOOLS" = false ]; then
	...
	if [ "$BUILD_TESTING" = true ]; then
		tools/travis/dm.sh -DTRAVISBUILDING -DTRAVISTESTING -DALL_MAPS riftstation.dme
	else
		tools/travis/dm.sh -DTRAVISBUILDING riftstation.dme && DreamDaemon riftstation.dmb -close -trusted -params "test-run&log-directory=travis"
